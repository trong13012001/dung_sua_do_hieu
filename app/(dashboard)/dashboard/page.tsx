      ...
      <div className="vuexy-card p-4 md:p-6 space-y-4 md:space-y-5">
        <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
          <div>
            ...
            <p className="text-xs md:text-sm text-muted-foreground mt-1">
              ...
              <span className="text-foreground/80">thời điểm khách nhận (trả đồ)</span>
            </p>
          </div>
        ...
        <p className="text-sm font-medium text-foreground">
          ...
          <span className="text-primary">
            {periodLabelVi(periodSelection.mode, periodSelection.value)}
          </span>
        </p>
          ...
          <>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
              ...
              <div className="rounded-xl border border-border bg-muted/10 p-3 md:p-4">
                ...
                <p className="text-[10px] text-muted-foreground mt-1">Dòng hàng mới trong kỳ</p>
              </div>
              <div className="rounded-xl border border-border bg-muted/10 p-3 md:p-4">
                <div className="flex items-center gap-2 text-muted-foreground text-xs font-semibold uppercase tracking-wide">
                  <Package size={14} />
                ...
                <p className="text-[10px] text-muted-foreground mt-1">Theo đơn trả trong kỳ</p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap gap-1.5 border-b border-border pb-2">
              {(
                [
                  ['itemsCreated', 'Hàng mới tạo', periodData.itemsCreated.length],
                  ['itemsReturned', 'Hàng đã trả', periodData.itemsReturned.length],
                  ['ordersCreated', 'Đơn tạo (chi tiết)', periodData.ordersCreated.length],
                  ['ordersReturned', 'Đơn đã trả (chi tiết)', periodData.ordersReturned.length],
                ] as const
              ).map(([key, label, n]) => (
                <button
                  key={key}
              ...
              <div className="overflow-x-auto max-h-[min(420px,55vh)] overflow-y-auto">
                {listTab === 'itemsCreated' && (
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-border">
                      {periodData.itemsCreated.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground italic">
                            Không có hàng nào được tạo trong kỳ này.
                          </td>
                        </tr>
                      ) : (
                        periodData.itemsCreated.map((row) => (
                          <tr key={row.id} className="hover:bg-muted/20">
                            <td className="px-4 py-2.5 font-medium text-foreground max-w-[200px]">
                              {row.item_name}
                            </td>
                            <td className="px-4 py-2.5">
                              <Link href="/orders" className="font-mono text-xs text-primary hover:underline">
                                #{String(row.order_id).padStart(5, '0')}
                              </Link>
                            </td>
                            <td className="px-4 py-2.5 text-muted-foreground">{row.customer_name}</td>
                            <td className="px-4 py-2.5">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${statusColor(row.status)}`}>
                  ...
                  <table className="w-full text-left text-sm">
                    <thead className="sticky top-0 z-[1] bg-muted/40 text-[11px] font-bold text-muted-foreground uppercase">
                      <tr>
                        ...
                        <th className="px-4 py-3">Khách</th>
                        <th className="px-4 py-3">Trạng thái</th>
                        <th className="px-4 py-3 whitespace-nowrap">Trả lúc</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-border">
                      {periodData.itemsReturned.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground italic">
                            Không có hàng nào thuộc đơn đã trả trong kỳ này.
                          </td>
                        </tr>
                      ) : (
                        periodData.itemsReturned.map((row) => (
                          <tr key={row.id} className="hover:bg-muted/20">
                            <td className="px-4 py-2.5 font-medium text-foreground max-w-[200px]">
                              {row.item_name}
                            </td>
                            ...
                            <td className="px-4 py-2.5 text-xs text-muted-foreground whitespace-nowrap">
                              {row.return_time
                                ? new Date(row.return_time).toLocaleString('vi-VN')
                                : '—'}
                            </td>
                          </tr>
                        ))
                ...
                {listTab === 'ordersCreated' && (
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-border">
                      {periodData.ordersCreated.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground italic">
                            Không có đơn nào được tạo trong kỳ này.
                          </td>
                        </tr>
                      ) : (
                        periodData.ordersCreated.map((o) => (
                          <tr key={o.id} className="hover:bg-muted/20">
                            ...
                            <td className="px-4 py-2.5">
                              <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${statusColor(o.status)}`}>
                ...
                {listTab === 'ordersReturned' && (
                  <table className="w-full text-left text-sm">
                    ...
                    <tbody className="divide-y divide-border">
                      {periodData.ordersReturned.length === 0 ? (
                        <tr>
                          <td colSpan={5} className="px-4 py-8 text-center text-muted-foreground italic">
                            Không có đơn nào được đánh dấu trả trong kỳ này.
                          </td>
                        </tr>
                      ) : (
                        periodData.ordersReturned.map((o) => (
                          <tr key={o.id} className="hover:bg-muted/20">
                            <td className="px-4 py-2.5">
                              <Link href="/orders" className="font-bold text-primary hover:underline">
                                #{String(o.id).padStart(5, '0')}
                            ...
                            <td className="px-4 py-2.5 text-xs text-muted-foreground whitespace-nowrap">
                              {o.return_time
                                ? new Date(o.return_time).toLocaleString('vi-VN')
                                : '—'}
                            </td>
                          </tr>
                        ))
            ...
            <p className="text-[11px] text-muted-foreground">
              Số đếm là đủ cho toàn bộ kỳ. Bảng chỉ hiển thị tối đa 150 đơn gần nhất và 200 hàng mới tạo / 400 hàng trong các đơn trả (theo thứ tự thời gian).
            </p>
          </>
        ) : null}
      </div>
